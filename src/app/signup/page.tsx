...
import axios, { AxiosError, isAxiosError } from 'axios'; // Import AxiosError and isAxiosError directly
import { useRouter } from 'next/navigation';
import GlobalBackground from '@/components/Landing/GlobalBackground';
import toast from 'react-hot-toast';

// --- Type Definitions ---
// Define a type for the expected error structure from your API
interface APIErrorResponse {
  message?: string;
  errors?: Record<string, string>; // For validation errors from the backend
}

// Define specific response types for your API calls
...
interface SignupResponse {
  success: boolean;
  message?: string; // Optional message for success or general error
  errors?: Record<string, string>; // Specific validation errors
}
...
export default function OnboardingPage() {
    ...
    const [otpError, setOtpError] = useState(false); // Renamed from otpError to avoid confusion with errors.otp
    ...
    const sendOTP = async () => {
      ...
      try {
        const { data } = await axios.post<SendOtpResponse>('/api/auth/send-otp', { email: formData.email });

        if (data.success) {
          setIsOTPSent(true);
          setOtpError(false); // Clear general OTP error state
        ...
        if (isAxiosError(err)) { // Use the imported isAxiosError
          errorMessage = err.response?.data?.message || err.message || 'Failed to send OTP. Network error?';
        }
    ...
    const verifyOTP = async () => {
      ...
      try {
        const { data } = await axios.post<VerifyOtpResponse>('/api/auth/verify-otp', { email: formData.email, otp });
        ...
        if (data.success) {
          ...
          setErrors({ ...errors, otp: data.message || 'Verification failed' });
          setOtpError(true); // Set general OTP error state
          toast.error(data.message || 'Verification failed');
        }
      } catch (err) {
        let errorMessage = 'Verification failed: An unexpected error occurred';
        if (isAxiosError(err)) {
          errorMessage = err.response?.data?.message || err.message || 'Verification failed. Network error?';
        }
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.errors) {
            ...
            toast.error(data.message || 'Please correct the specific errors.');
          } else {
        ...
        if (isAxiosError(err)) {
          const axiosError = err as AxiosError<APIErrorResponse>; // Cast to AxiosError with APIErrorResponse
          if (axiosError.response?.data?.message) {
            errorMessage = axiosError.response.data.message;
          }
          if (axiosError.response?.data?.errors) {
            backendErrors = axiosError.response.data.errors;
            // Merge backend errors with existing form errors
            setErrors(prevErrors => ({ ...prevErrors, ...backendErrors }));
          }
        }

        if (Object.keys(backendErrors).length === 0) { // Only show general toast if no specific errors were set
             toast.error(errorMessage, {
                duration: 4000,
                position: 'top-center',
            });
        ...
        <div className="relative min-h-screen flex flex-col items-center justify-center p-4">
            ...
            >
                ...
                >
                    ...
                    {/* Display General Error Message at bottom */}
                    {(Object.values(errors).some(Boolean) && !errors.otp) && // Show general error if any field error exists, but not specifically OTP
                        <motion.p
