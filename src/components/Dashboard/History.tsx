...
export const History = () => {
  ...
  // Helper function to get agent-specific info for display 
  ...
  return (
    ...
    >
      ...
      {/* Search Input - Optional, can be uncommented if needed */}
      <div className="flex items-center justify-end gap-3">
        ...
        <div className="relative w-1/2">
          <input
            type="text"
            placeholder="Search..."
            className={`w-full rounded-2xl px-4 py-2 pr-10 transition-all duration-200 border ${isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-800'} 
            ${sectionBorderClass} focus:ring-2 focus:ring-blue-500 focus:outline-none`}
            value={searchTerm}
          ...
          {/* Search Icon Button */}
          <button
            className="absolute right-2 top-1/2 -translate-y-1/2 text-emerald-500 hover:text-emerald-700 cursor-pointer"
            onClick={() => {
        ...
        <motion.button
          ...
          onClick={() => {
            setSearchTerm('');
            getHistory(0, false); // Reload full history
          }}
          title='Reset filters'
          className={`inline-flex items-center justify-center gap-2 px-3 py-2 rounded-full text-white text-sm font-medium
                    transition-all duration-300 ease-in-out ${primaryButtonGradient} ${primaryButtonShadow} cursor-pointer`}
        >
          {loadingHistory ? (
